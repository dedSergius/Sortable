...
use PDO;
use PDOException;
use App\Utility\MareiObj;
use App\Utility\MareiCollection;
use Exception;

//====================== Marei DB Class V 1.0 (Modified by Sergey Lukin specifically for this project) ======================
class Database{
	...
	private function __construct()
	{
	    $db_config = [
            ...
            //Localhost
            "development" => [
                "host" => Config::get("DEV_DATABASE_HOST"),
                "database" => Config::get("DEV_DATABASE_NAME"),
                "username" => Config::get("DEV_DATABASE_USERNAME"),
                "password" => Config::get("DEV_DATABASE_PASSWORD")
            ],
            //Server
            "production"  => [
	...
	public function get()
	{
		...
		$rows = $stmt->fetchAll(PDO::FETCH_CLASS,'App\Utility\MareiObj');
		$collection= [];
		$collection = new MareiCollection;
		$x=0;
		foreach ($rows as $key => $row) {
